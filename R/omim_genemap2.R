...
#' @export
get_omim_genemap2 <- function(genemap2_file)
{
    if (!file.exists(genemap2_file)) {
        stop("Provided OMIM genemap2 file does not exist. ", 
             "Download genemap2.txt from https://omim.org/downloads/ and specify file location.")
    } 
    
    omim_genemap2 <- cavalier_cache$omim_genemap2
    
    if (is.null(omim_genemap2)) {
        
        genemap2_file <- normalizePath(genemap2_file)
        ...
        omim_genemap2 <- 
            (function() read_tsv(genemap2_file, col_names = col_names,
                                 comment = '#', col_types = cols())) %>% 
            cache(str_c('genemap2_', digest::digest(genemap2_file))) %>% 
         ...
         cavalier_cache$omim_genemap2 <- omim_genemap2
    }
